...
```js
var myDID = wallet.createDID(DIDNetwork.PrivateNet); // e.g. did:sid:priv:AKkkumHbBipZ46UMZJoFynJMXzSRnBvKcs
```
...
var allClaims = wallet.getAllClaims('did:sid:priv:AKkkumHbBipZ46UMZJoFynJMXzSRnBvKcs');
...
```js
var claim = issuer.createClaim('claimId', 'schemaName', {'firstName': 'John', 'lastName': 'Doe', 'age': 26}, 'did:sid:priv:AKkkumHbBipZ46UMZJoFynJMXzSRnBvKcs');
...
```js
rot.registerIssuer('did:sid:priv:AKkkumHbBipZ46UMZJoFynJMXzSRnBvKcs', 'SchemaName', 'rootOfTrustPrivateKey');
```

Remove trust for issuer's DID and schema from RoT:
```js
rot.deactivateIssuer('did:sid:priv:AKkkumHbBipZ46UMZJoFynJMXzSRnBvKcs', 'SchemaName', 'rootOfTrustPrivateKey');
```

Check if issuer is trusted with the given schema:
```js
var trusted = rot.isTrusted('did:sid:priv:AKkkumHbBipZ46UMZJoFynJMXzSRnBvKcs', 'SchemaName');
```
